...
// #define OMP_THREADS omp_get_num_threads()
#define OMP_THREADS atoi(getenv("OMP_NUM_THREADS"))
...
int main(int argc, char** argv) {
	...
	if (!getenv("SKIP_BFS")) {
		...
		int64_t total_edge_visit_count = 0;
			
		for (_bfs_root_idx = 0; _bfs_root_idx < num_bfs_roots; ++_bfs_root_idx) {
			...
			#pragma omp parallel
			{
				run_bfs(bfs_roots[0], &pred[0]);
			}
			double bfs_stop = MPI_Wtime();
			bfs_times[_bfs_root_idx] = bfs_stop - bfs_start;
			if (rank == 0) fprintf(stderr, "Time for BFS %d is %f\n", _bfs_root_idx, bfs_times[_bfs_root_idx]);
			int64_t edge_visit_count=0;
			get_edge_count_for_teps(&edge_visit_count);
			edge_counts[_bfs_root_idx] = (double)edge_visit_count;

			if (rank == 0) fprintf(stderr, "TEPS for BFS %d is %g\n", _bfs_root_idx, edge_visit_count / bfs_times[_bfs_root_idx]);
			...
			if (!getenv("SKIP_VALIDATION")) {
				...
				int validation_passed_one = validate_result(1,&tg, nlocalverts, bfs_roots[0], pred,shortest,&edge_visit_count);
				double validate_stop = MPI_Wtime();

				validate_times[_bfs_root_idx] = validate_stop - validate_start;
				if (rank == 0) fprintf(stderr, "Validate time for BFS %d is %f\n", _bfs_root_idx, validate_times[_bfs_root_idx]);
				...
				validate_times[_bfs_root_idx] = -1;
		}
